...
def generate_plot(tool_name, file_name, exclude_training_snps=False):
    column1 = tool_name + PEARSON_CORELATION_SUFFIX
    ...
    dict_df = {k.replace(PEARSON_CORELATION_SUFFIX, ""): v for k, v in sorted_df.abs().to_dict('list').items()}
...
if __name__ == '__main__':
    ...
    baseline_column  = COLUMN_NAME_OF_BASELINE_SCORES_DICTIONARY + PEARSON_CORELATION_SUFFIX
    ...
    column1 = MUTEPRED_TOOL_NAME + PEARSON_CORELATION_SUFFIX + EXCLUDE_TRAINING_SNP_SUFFIX
    column2 = DEOGEN_TOOL_NAME + PEARSON_CORELATION_SUFFIX + EXCLUDE_TRAINING_SNP_SUFFIX
    ...
    column_names = [id_column, baseline_column] + [tool + PEARSON_CORELATION_SUFFIX for tool in TOOLS_LIST]
    sorted_df = LOADED_MAVE_DF.loc[:, column_names].sort_values(by=baseline_column, ascending=True)
